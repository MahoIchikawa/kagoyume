<?php
 ...
 //TOPpageログ
 $log_content = $ses_name. " move to Searching page ";
 writeLog("SEARCH", $log_content);
...
setcookie("category_id", $category_id, time() + 3600);
setcookie("search_term", $query, time() + 3600);
...
if ($query != "") {
 ...
 <html>
  ...
  <body>
      ...
      <?php
           if(count($hits) <= 0){
             echo "<h3 class='text-info'>There are no results that match your search ...!<h3>";
             die;
           }
           for ($i = 0; $i <12; $i++) {
             $hit = $hits[$i];
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-12 col-md-6">
                          <form action=" <?php echo ADD ?>" method="POST">
                            <button type="submit" class ="btn btn-success" name="add_to_cart" value="<?php echo $hit->Code?>"> Add to cart</a>
